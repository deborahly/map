...
class App {
  ...
  _loadMap(position) {
    ...
    this.#workoutArray.forEach(workout => this._renderWorkoutMarker(workout));
  }
  ...
  _newWorkout(e) {
    ...
    //   inputs.reduce(function (accum, input) {
  ...
  _moveToPopup(e) {
    ...
    const workObj = this.#workoutArray.find(
      workout => workout.id === workEl.dataset.id
    );

    this.#map.setView(workObj.coords, this.#mapZoomLevel, {
      animation: true,
      pan: {
        duration: 1,
      },
  ...
  _getLocalStorage() {
    ...
    this.#workoutArray.forEach(workout => this._renderWorkout(workout));
  }
